	...
	if (inputs.length > 0) {
		for (let index = 0; index < inputs.length; index++) {
			...
			input.addEventListener('focus', function (e) {
				...
				if (input.classList.contains('_cyrillic')) {
					input.classList.add('_mask');
					Inputmask({ regex: "[А-Яа-яЁё ]*"}, {
						//"placeholder": '',
						clearIncomplete: true,
						clearMaskOnLostFocus: true,
						onincomplete: function () {
...
window.onmousewheel = document.onmousewheel = window.addEventListener('scroll', scroll_scroll);
  ...
  if (scr_items.length > 0) {
    for (let index = 0; index < scr_items.length; index++) {
      ...
      if ((src_value > scr_item_offset - scr_item_point) && src_value < (scr_item_offset + scr_item_height)) {
        ...
        if (!scr_item.classList.contains('_scr-no-hide')) {
...
if (link) {
  ...
  for (let index = 0; index < link.length; index++) {
    ...
    el.addEventListener('click', function (e) {
      ...
      let target_block_value = document.querySelector('.' + target_block_class);
      let target_block = target_block_value.getBoundingClientRect().top + pageYOffset - document.querySelector('header').offsetHeight;
...
if (goto_links) {
  for (let index = 0; index < goto_links.length; index++) {
    ...
    goto_link.addEventListener('click', function (e) {
      ...
      let target_block_value = target_block.getBoundingClientRect().top + pageYOffset - document.querySelector('header').offsetHeight;
      _goto(target_block_value, 300);
      e.preventDefault();
    });
  }
}
